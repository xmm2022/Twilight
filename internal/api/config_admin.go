...
const telegramGroupUserPanelTemplateDescription = "自定义 /twguser 群组用户面板文本，支持换行；留空使用内置模板。安全限制：不会提供邮箱、Emby ID、Telegram ID、密码、Token 或服务器线路占位符。" +
	...
	"{telegram_status}=Telegram 绑定摘要；{telegram_username}=Telegram 用户名；{emby_status}=本地 Emby 绑定摘要（含用户名）；{emby_bound_status}=本地 Emby 绑定状态（不含用户名）；{emby_bound}=是否已绑定 Emby；{emby_username}=本地 Emby 用户名；{emby_unbind_allowed}=是否允许用户自助解绑 Emby；" +
