...
| URL/颜色/尺寸白名单 | 同上 |
| 邀请码 IP 限流（公开校验） | `src/api/v1/invite.py::check_invite_code` |
| 邀请人/被邀请人自洽校验 | `src/services/invite_service.py::apply_invite` |
| 删除用户自动清理邀请关系，避免悬空 | `src/services/user_service.py::delete_user` |
